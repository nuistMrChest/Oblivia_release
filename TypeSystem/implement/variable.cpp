...
namespace Oblivia{
	...
	Function&ValueType::Fun(){
		return*std::get<std::unique_ptr<Function>>(v);
	}
	...
	const Function&ValueType::Fun()const{
		return*std::get<std::unique_ptr<Function>>(v);
	}
	...
	ValueType::ValueType(const ValueType&a){
		if(std::holds_alternative<Number>(a.v))
			this->v=a.Num();
		else if(std::holds_alternative<std::unique_ptr<Array>>(a.v))
			this->v=std::make_unique<Array>(a.Arr());
		else if(std::holds_alternative<std::unique_ptr<Object>>(a.v))
			this->v=std::make_unique<Object>(a.Obj());
		else if(std::holds_alternative<std::unique_ptr<String>>(a.v))
			this->v=std::make_unique<String>(a.Str());
		else if(std::holds_alternative<std::unique_ptr<Reference>>(a.v))
			this->v=std::make_unique<Reference>(a.Ref());
		else if(std::holds_alternative<std::unique_ptr<Function>>(a.v))
			this->v=std::make_unique<Function>(a.Fun());
	...
	Variable::Variable(size_t l,const std::string&n,const Function&v):name(n),scope_level(l){
		as.v=std::make_unique<Function>(v);
		type=Type::Function;
		calculatable=false;
		variables[VarKey(n,l)]=this;
	}
