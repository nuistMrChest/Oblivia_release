...
namespace Oblivia{
	...
	ArrayElement::ArrayElement(const Array&f,size_t i,const std::unique_ptr<Function>&a):index(i),father(&f){
		type=Type::Function;
	...
	Array::Array(const Array&a){
		...
		for(int i=0;i<a.array.size();i++){
			switch(a.array[i]->getType()){
				...
				case Type::Function:this->array[i]=std::make_unique<ArrayElement>(*this,i,std::make_unique<Function>(a.array[i]->as.Fun()));break;
			}
		}
	}
	...
	Number ArrayElement::operator==(const ArrayElement&a)const{
		...
		switch(a.type){
			...
			case Type::Function:return Number(0);
