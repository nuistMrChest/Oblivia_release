...
namespace Oblivia{
	...
	Attribute::Attribute(const std::string&n,const Object&f,int i,const std::unique_ptr<Function>&a):index(i),father(&f),name(n){
		type=Type::Function;
		as.v=std::make_unique<Function>(*a);
		calculatable=false;
	}
	...
	std::ostream&operator<<(std::ostream&os,const Attribute&a){
		...
		switch(a.type){
			...
			case Type::Function:os<<a.as.Fun();break;
		}
		return os;
	}
	...
	Object::Object(const Object&a){
		...
		for(int i=0;i<a.attributes.size();i++){
			switch(a.attributes[i]->getType()){
				...
				case Type::Function:this->attributes[i]=std::make_unique<Attribute>(a.attributes[i]->name,*this,i,std::make_unique<Function>(a.attributes[i]->as.Fun()));break;
			}
		}
	}
	...
	void Object::addAttribute(const std::string&name,const std::unique_ptr<Function>&a){
		int index=attributes.size();
		attributes.push_back(std::make_unique<Attribute>(name,*this,index,a));
		getIndex[name]=index;
	}
